...
class TestSurveyConditionalQuestions(common.SurveyCase, HttpCase):
    def test_survey_conditional_questions(self):
        question = self._add_question(
            self.page_0, "Q0", "numerical_box", survey_id=self.survey.id
        )
        ...
        conditional_values = {
            ...
            "conditional_minimum_value": 1,
            "conditional_maximum_value": 10,
        }
        ...
        self.env["survey.user_input_line"].save_lines(
            user_input.id, question, {tag: "2"}, tag
        )
        ...
        self.env["survey.user_input_line"].save_lines(
            user_input.id, question, {tag: "11"}, tag
        )
        user_input.refresh()
        self.assertIn(conditional_question, user_input.get_hidden_questions())
        ...
        self.env["survey.user_input_line"].save_lines(
            ...
            {conditional_tag: conditional_question.labels_ids[1]["value"], tag: "11"},
            conditional_tag,
        )
        user_input.refresh()
