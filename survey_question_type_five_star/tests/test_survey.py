...
class TestSurvey(TransactionCase):
    ...
    def setUp(self):
        ...
        (group_survey_user, group_employee) = (
            self.ref('survey.group_survey_user'), self.ref('base.group_user'))
        self.survey_manager = User.create({
            'name': 'Gustave Doré', 'login': 'Gustav',
            'email': 'gustav.dore@example.com',
            'groups_id': [(6, 0, [self.ref('survey.group_survey_manager'),
                                  group_survey_user, group_employee])]})
        self.survey1 = self.env['survey.survey'].sudo(
            self.survey_manager).create(
            {'title': "S0", 'page_ids': [(0, 0, {'title': "P0"})]})
        self.page1 = self.survey1.page_ids[0]
        self.user_input1 = self.env['survey.user_input'].sudo(
            self.survey_manager).create(
            {'survey_id': self.survey1.id,
             'partner_id': self.survey_manager.partner_id.id}
        )
        self.question1 = self.env['survey.question'].\
            sudo(self.survey_manager).create({
                'page_id': self.page1.id,
                'question': 'Q0',
                'type': 'star_rate',
                'validation_required': True, })
        self.answer_tag1 = '%s_%s_%s' % (self.survey1.id,
                                         self.page1.id, self.question1.id)
    ...
    def test_01_question_star_rate_with_error_values(self):
        ...
        for i in range(len(error_results)):
            self.assertEqual(self.question1.validate_question(
